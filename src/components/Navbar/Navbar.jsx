...
const Navbar = ({itemStyle, setItemStyle}) => {
  ...
  return (
    <div className={style.container}>
      ...
      <GenreSelect genreList={genreList} onClickFilmsByGenreId={onClickFilmsByGenreId}/>
      <div className={style.switchingStyle}>
        <BsFillGrid3X3GapFill onClick={()=>setItemStyle(false)} className={itemStyle ? style.active : style.inactive}/>
        <FaThList onClick={()=>setItemStyle(true)} className={itemStyle ? style.inactive : style.active}/>
      </div>
    </div>)
}
